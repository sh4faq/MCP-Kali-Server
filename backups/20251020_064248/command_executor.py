...
def execute_command_argv(argv, timeout=300, on_output=None, env=None):
    """
    Safer, more robust executor that accepts a list of args (argv).
    Runs with shell=False, streams output line-by-line if on_output is provided.
    Returns the same shape as execute_command to keep callers consistent.
    """
    import time
    start = time.time()
    proc = None
    ...
    try:
        ...
        def _pump(stream, sink, source):
            for line in iter(stream.readline, ''):
                sink.append(line)
                if on_output:
                    try:
                        on_output(source, line.rstrip("\n"))
                    except Exception:
                        pass
            stream.close()
