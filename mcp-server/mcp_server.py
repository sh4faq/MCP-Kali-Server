...
def setup_mcp_server(kali_client: KaliToolsClient) -> FastMCP:
    ...
    @mcp.tool()
    def tools_waybackurls(target: str, additional_args: str = "") -> Dict[str, Any]:
        """
        Execute waybackurls to fetch archived URLs from the Wayback Machine.
        
        Args:
            target: Domain to fetch archived URLs for (e.g., example.com)
            additional_args: Additional arguments (e.g., "-dates" to show fetch dates, "-no-subs" to exclude subdomains)
            
        Returns:
            Execution results including archived URLs
        """
        data = {
            "domain": target,
            "additional_args": additional_args
    ...
    def tools_byp4xx(
        ...
        """
        Execute byp4xx to attempt bypassing 403/401/402 Forbidden responses.
        byp4xx is a fast and effective tool for bypassing HTTP 4xx status codes.
        
        Args:
            url: The target URL returning 403/401/402
            additional_args: Additional byp4xx arguments
            verbose: Show all bypass attempts (--all flag)
            threads: Number of threads to use (-t flag)
            rate: Rate limit for requests (default: 5 requests/second)
            
        Returns:
            Bypass attempt results with 2000 second timeout
        """
        data = {
            "url": url,
            "additional_args": additional_args,
    ...
    def tools_ffuf(
        ...
        """
        Execute ffuf web fuzzer for content discovery and fuzzing.
        
        Args:
            url: The target URL (must contain FUZZ keyword for fuzzing position)
            wordlist: Path to wordlist file
            method: HTTP method (GET, POST, etc.)
            headers: Custom headers (semicolon-separated for multiple headers)
            data: POST data string (can also contain FUZZ keyword)
            additional_args: Additional ffuf arguments (e.g., "-mc 200,301 -fc 404")
