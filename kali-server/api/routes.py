...
from tools.kali_tools import (
    ...
    run_metasploit, run_hydra, run_john, run_wpscan, run_enum4linux, run_ffuf,
    run_subfinder, run_httpx, run_searchsploit, run_nuclei, run_arjun,
...
def setup_routes(app: Flask):
    ...
    @app.route("/api/tools/bypass_403", methods=["POST"])
    def bypass_403():
        try:
            params = request.json or {}
            ...
            logger.error(f"Error in bypass_403 endpoint: {str(e)}")
            return jsonify({"error": f"Server error: {str(e)}"}), 500
