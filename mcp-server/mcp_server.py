...
DEFAULT_KALI_SERVER = "http://192.168.229.130:5000" # change to your linux IP
...
def setup_mcp_server(kali_client: KaliToolsClient) -> FastMCP:
    ...
    @mcp.tool()
    def tools_fierce(domain: str, dns_server: str = "", wordlist: str = "", additional_args: str = "") -> Dict[str, Any]:
        """
        Execute Fierce DNS enumeration tool.
        
        Args:
            domain: Target domain to enumerate
            dns_server: Specific DNS server to use
    ...
    def tools_403bypasser(
        ...
        """
        Execute 403bypasser to bypass 403 Forbidden responses.
        
        Args:
            url: Single URL to scan (e.g., http://example.com)
            urllist: Path to file containing list of URLs
            directory: Single directory to scan (e.g., /admin)
            dirlist: Path to file containing list of directories
            additional_args: Additional 403bypasser arguments
        ...
        return kali_client.safe_post("api/tools/403bypasser", data)
    ...
    def tools_httpx(target: str, additional_args: str = "") -> Dict[str, Any]:
        ...
        Args:
            target: The target URL or path to file containing URLs
            additional_args: Additional httpx arguments (e.g., "-status-code -title -tech-detect")
    ...
    def tools_searchsploit(query: str, additional_args: str = "") -> Dict[str, Any]:
        ...
        Args:
            query: Search query (e.g., "apache 2.4", "wordpress")
    ...
    def tools_nuclei(
        ...
        Args:
            ...
            templates: Path to templates or template tags (e.g., "-t cves/", "-t exposures/")
    ...
    def tools_subzy(
        ...
        """
        Execute Subzy for subdomain takeover vulnerability detection.
        
        Args:
            target: Single subdomain to check (e.g., "test.example.com")
            targets_file: Path to file containing list of subdomains
            additional_args: Additional Subzy arguments (e.g., "--https --concurrency 20")
            
        Returns:
            Subdomain takeover vulnerability scan results
        """
        data = {
            "target": target,
    ...
    def tools_waybackurls(domain: str, additional_args: str = "") -> Dict[str, Any]:
        """
        Execute waybackurls to fetch URLs from the Wayback Machine.
        
        waybackurls fetches all URLs that the Wayback Machine (archive.org) knows about 
        for a domain. This is extremely useful for:
        - Discovering old endpoints that may still be active
        - Finding hidden or forgotten pages and directories
        - Locating backup files, old admin panels, or test pages
        - Reconnaissance and attack surface mapping
        
        Args:
            domain: The target domain (e.g., "example.com")
            additional_args: Additional waybackurls arguments:
                           - "-dates" to show fetch dates
                           - "-get-versions" to list URLs for crawled versions
                           - "-no-subs" to exclude subdomains
            
        Returns:
            List of archived URLs from the Wayback Machine
        """
        data = {
            "domain": domain,
            "additional_args": additional_args
